...
IZIN=$(curl -sS http://lostserver.my.id:81/akses | grep $MYIP) >/dev/null 2>&1
if [ $MYIP = $IZIN ]; then
...
if [[ ${NUMBER_OF_CLIENTS} == '0' ]]; then
    ...
    echo -e "\e[41m            Delete Trojan Account        \E[0m"
    echo -e "\033[1;96m─────────────────────────────────────────\033[0m"
    echo ""
...
systemctl restart xray@trojan-ws.service
systemctl restart xray@trojan-grpc.service
clear
echo -e "\033[1;96m─────────────────────────────────────────\033[0m"
