...
if [[ ${NUMBER_OF_CLIENTS} == '0' ]]; then
    ...
    echo -e "\e[41m         Renew Xray/Shadowsocks Account  \E[0m"
    echo -e "\033[1;96m─────────────────────────────────────────\033[0m"
    echo ""
...
echo -e "\033[1;96m─────────────────────────────────────────\033[0m"
echo -e "\e[41m         Renew Xray/Shadowsocks Account  \E[0m"
echo -e "\033[1;99m─────────────────────────────────────────\033[0m"
echo ""
grep -E "^### " "/etc/xray/ss-ws.json" | cut -d ' ' -f 2-3 | column -t | sort | uniq
echo ""
...
if [ -z $user ]; then
    ...
    exp=$(grep -wE "^### $user" "/etc/xray/ss-ws.json" | cut -d ' ' -f 3 | sort | uniq)
    now=$(date +%Y-%m-%d)
    d1=$(date -d "$exp" +%s)
    d2=$(date -d "$now" +%s)
    exp2=$(((d1 - d2) / 86400))
    ...
    sed -i "/### $user/c\### $user $exp4" /etc/xray/ss-ws.json
    sed -i "/### $user/c\### $user $exp4" /etc/xray/ss-grpc.json
